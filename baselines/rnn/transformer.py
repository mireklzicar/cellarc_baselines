...
from . import positional_encodings as pos_encs_lib
...
class MultiHeadDotProductAttention(nn.Module):
  ...
  def forward(
    ...
    if self._attention_window is not None:
      ...
      attention = torch.where(attention_mask, attention,
...
class TransformerEncoder(nn.Module):
  ...
  def __init__(
    ...
    if config.use_embeddings and shared_embeddings_fn is None:
      self.embeddings = nn.Linear(config.output_size, config.embedding_dim, bias=False)
