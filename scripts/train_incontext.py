...
def evaluate(
    ...
    *,
    ...
    if vote_mode not in {"mean_logits", "majority"}:
        raise ValueError(f"Unsupported TTA vote mode '{vote_mode}'. Expected 'mean_logits' or 'majority'.")
    if apply_to_mode not in {"query_only", "both"}:
        raise ValueError(f"Unsupported TTA apply_to='{apply_to_mode}'. Expected 'query_only' or 'both'.")
    ...
    with torch.no_grad():
        for batch_index, batch in enumerate(loader):
            ...
            if not tta_enabled:
                ...
                base_logits = forward_batch(model, batch)
                logits_sum = base_logits.clone()
                preds_views: List[torch.Tensor] = [base_logits.argmax(dim=-1)]
                num_views = 1

                aggregate_mask = valid_mask.clone()
                if apply_to_mode == "query_only" and loss_mask is not None:
                    aggregate_mask = aggregate_mask & loss_mask

                # Ensure aggregation covers only valid positions.
                aggregate_mask = aggregate_mask & valid_mask
                ...
                for aug_idx in range(num_augs):
                    ...
                    logits_aug = forward_batch(model, aug_batch)
                ...
                if aggregate_mask.any() and num_views > 1 and vote_mode == "majority":
                    ...
                    if votes.numel() > 0:
                        ...
                        final_predictions = final_preds_flat.view_as(preds_views[0])
                    else:
                        final_predictions = preds_views[0]
                else:
                ...
                effective_loss = per_token_loss * valid_mask.float()
                total_loss += float(effective_loss.sum().item())
                valid_tokens = int(valid_mask.long().sum().item())
                if valid_tokens > 0:
                    total_tokens += valid_tokens
                    correct = (final_predictions == labels) & valid_mask
...
def aggregate_split_metrics(
    ...
    for key in summed_keys:
        total = sum(metrics.get(key, 0.0) for metrics in metrics_by_split.values())
        if total:
...
def log_split_metrics(
    *,
    ...
    for split_name in sorted(metrics_by_split):
        ...
        message = (
            f"{tag}{epoch_str} split={split_name} step={step} "
            f"loss={metrics.get('loss', 0.0):.4f} "
            f"acc={metrics.get('accuracy', 0.0) * 100.0:.2f}% "
            f"episodes={int(metrics.get('episodes', 0.0))} "
            f"tokens={int(metrics.get('tokens', 0.0))}"
        )
        if "support/loss" in metrics:
            message += (
                f" support_loss={metrics['support/loss']:.4f}"
                f" support_acc={metrics.get('support/accuracy', 0.0) * 100.0:.2f}%"
            )
        if "query/loss" in metrics:
            message += (
                f" query_loss={metrics['query/loss']:.4f}"
                f" query_acc={metrics.get('query/accuracy', 0.0) * 100.0:.2f}%"
            )
        LOGGER.info(message)
    if aggregated_metrics:
        agg_message = (
            f"{tag}{epoch_str} split=mean step={step} "
            f"loss={aggregated_metrics.get('loss', 0.0):.4f} "
            f"acc={aggregated_metrics.get('accuracy', 0.0) * 100.0:.2f}% "
            f"episodes={int(aggregated_metrics.get('episodes', 0.0))} "
            f"tokens={int(aggregated_metrics.get('tokens', 0.0))}"
        )
        if "support/loss" in aggregated_metrics:
            agg_message += (
                f" support_loss={aggregated_metrics['support/loss']:.4f}"
                f" support_acc={aggregated_metrics.get('support/accuracy', 0.0) * 100.0:.2f}%"
            )
        if "query/loss" in aggregated_metrics:
            agg_message += (
                f" query_loss={aggregated_metrics['query/loss']:.4f}"
                f" query_acc={aggregated_metrics.get('query/accuracy', 0.0) * 100.0:.2f}%"
            )
        LOGGER.info(agg_message)
    ...
    for split_name in metrics_by_split:
        ...
        if "support/loss" in metrics:
            payload[f"{tag}/support_loss/{split_name}"] = float(metrics.get("support/loss", 0.0))
            payload[f"{tag}/support_accuracy/{split_name}"] = float(metrics.get("support/accuracy", 0.0))
        if "query/loss" in metrics:
            payload[f"{tag}/query_loss/{split_name}"] = float(metrics.get("query/loss", 0.0))
            payload[f"{tag}/query_accuracy/{split_name}"] = float(metrics.get("query/accuracy", 0.0))
    if aggregated_metrics:
        ...
        if "support/loss" in aggregated_metrics:
            payload[f"{tag}/support_loss_mean"] = float(aggregated_metrics.get("support/loss", 0.0))
            payload[f"{tag}/support_accuracy_mean"] = float(aggregated_metrics.get("support/accuracy", 0.0))
        if "query/loss" in aggregated_metrics:
            payload[f"{tag}/query_loss_mean"] = float(aggregated_metrics.get("query/loss", 0.0))
            payload[f"{tag}/query_accuracy_mean"] = float(aggregated_metrics.get("query/accuracy", 0.0))
        payload[f"{tag}/episodes_total"] = float(aggregated_metrics.get("episodes", 0.0))
        payload[f"{tag}/tokens_total"] = float(aggregated_metrics.get("tokens", 0.0))
...
def sample_palette_perm(rng: random.Random, num_colors: int) -> List[int]:
    """Return a random permutation over colour token ids."""
...
def apply_palette(
    ...
    """Apply a palette permutation to colour tokens."""

    if num_colors <= 0:
        return x
    out = x.clone()
    lut = torch.tensor(perm, dtype=out.dtype, device=out.device)
    mask = out < num_colors
    if mask.any():
...
def reverse_value_segments(
    x: torch.Tensor,
    tokens: TokenVocabulary,
) -> torch.Tensor:
    """Reverse contiguous runs of value tokens between marker tokens."""

    out = x.clone()
    markers = {
        tokens.support_input_id,
        tokens.support_output_id,
        tokens.query_id,
        tokens.target_id,
        tokens.eos_id,
    }
    ...
    for b_idx in range(batch_size):
        pos = 0
        while pos < seq_len:
            current = int(out[b_idx, pos].item())
            if current in markers:
                end = pos + 1
                while (
                    end < seq_len
                    and int(out[b_idx, end].item()) not in markers
                    and int(out[b_idx, end].item()) != tokens.pad_id
                ):
                    end += 1
                if end > pos + 1:
                    span = out[b_idx, pos + 1 : end]
                    out[b_idx, pos + 1 : end] = torch.flip(span, dims=[0])
                pos = end
            else:
                pos += 1
    return out
...
def run(cfg: DictConfig) -> None:
    ...
    architecture_name = str(cfg.model.architecture)
    recursive_architectures = {"tiny_recursive", "trm", "hrm", "transformer_act"}
    is_recursive_model = architecture_name in recursive_architectures

    recursive_cfg = cfg.get("recursive_trainer", {})
    if is_recursive_model:
        rec_lr = float(recursive_cfg.get("lr", 1e-4))
        beta1 = float(recursive_cfg.get("beta1", 0.9))
        beta2 = float(recursive_cfg.get("beta2", 0.999))
        rec_weight_decay = float(
        ...
        scheduler = None  # TODO: add cosine schedule with warmup when metadata available.
    else:
        optimizer_name = str(cfg.optimizer.name).lower()
        if optimizer_name != "adamw":  # pragma: no cover - config validation
            raise ValueError(
                f"Unsupported optimizer '{cfg.optimizer.name}'. Only AdamW is implemented."
            )
    ...
    if is_recursive_model:
        recursive_cfg = cfg.get("recursive_trainer", {})
        grad_acc = recursive_cfg.get("gradient_accumulation", cfg.trainer.gradient_accumulation)
        gradient_accumulation = max(1, int(grad_acc))
    else:
