#!/usr/bin/env python3
"""Download train/val accuracy curves for all W&B runs and save a single CSV."""
...
def collect_run_accuracy_history(
    ...
    *,
    ...
    """Collect per-step train/val accuracy for a single run."""
    ...
    for row in history_iter:
        ...
        if step is None:
            continue
        train_accuracy = first_matching_value(row, TRAIN_ACCURACY_KEYS, "train/accuracy")
        val_accuracy = first_matching_value(row, VAL_ACCURACY_KEYS, "val/accuracy")
        if train_accuracy is None and val_accuracy is None:
            continue
...
def collect_all_accuracy_curves(entity: str, projects: Sequence[str]) -> pd.DataFrame:
    """Fetch every run's accuracy history for the provided projects."""
    api = wandb.Api()
    ...
    for project in projects:
        ...
        for run in runs:
            ...
            try:
                records = collect_run_accuracy_history(
                    run,
                    project=project,
                    training_mode=training_mode,
                    size=size,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fetch train/val accuracy curves for all W&B runs.")
    parser.add_argument(
        "--entity",
        default=ENTITY,
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=Path("results/wandb_export/all_wandb_accuracy_curves.csv"),
        help="Destination CSV file (default: results/wandb_export/all_wandb_accuracy_curves.csv).",
    )
...
def main() -> None:
    args = parse_args()
    df = collect_all_accuracy_curves(args.entity, args.projects)
    if df.empty:
        print("No accuracy metrics found for the requested projects.")
        return
