#!/usr/bin/env python3
"""Plot per-project accuracy curves from exported W&B history."""
...
DEFAULT_INPUT = Path("results/wandb_export/all_wandb_accuracy_curves.csv")
DEFAULT_OUTPUT_DIR = Path("results/wandb_export/accuracy_plots")
...
OVERVIEW_FILENAME = "accuracy_curves_overview.png"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot accuracy curves for every project/run combination.")
    parser.add_argument(
        "--input-csv",
        type=Path,
        default=DEFAULT_INPUT,
        help=f"CSV produced by analysis/get_wandb_accuracy.py (default: {DEFAULT_INPUT}).",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def pretty_project_title(project: str, training_mode: Optional[str], size: Optional[str]) -> str:
    ...
    if isinstance(training_mode, str) and training_mode:
        ...
        descriptor = "Accuracy Curves"
    return f"Accuracy Curves of {size_label} {descriptor}"
...
def plot_project_metric(
    ...
    ax.set_title(f"{title} — {metric.title()} Accuracy")
    ax.set_xlabel("Step")
...
def plot_project(
    ...
    for metric in metrics:
        ...
        if plot_project_metric(df, metric, output_path, color_map, label_map, dpi):
            ...
            print(f"Skipped {df['project'].iloc[0]} ({metric}): no {metric} accuracy values.")
    return saved
...
def plot_overview_grid(
    ...
    for row_idx, project_name in enumerate(project_order):
        ...
        for col_idx, metric in enumerate(metrics):
            ...
            if row_idx == 0:
                title = "Train Accuracy" if metric == "train" else "Validation Accuracy"
                ax.set_title(title)
...
def main() -> None:
    ...
    if not args.input_csv.exists():
        raise SystemExit(f"{args.input_csv} does not exist. Run analysis/get_wandb_accuracy.py first.")
    ...
    if saved == 0:
        raise SystemExit("No figures were generated; check that the CSV has train/val accuracy values.")
